...
main_dir="/vast/palmer/scratch/ohern/as4643"
# Define the main folder containing subfolders to be run
main_folder="${main_dir}/Decoys/Supersampled_structures/groups"

# Define the sbatch file
sbatch_hydro="${main_dir}/Protein-Decoy-Detection-SVR/sbatch_hydro.sh"
...
for subfolder in "${main_folder}"/*/; do
    ...
    sed -i "s|^#SBATCH --job-name=.*|#SBATCH --job-name=${subfolder_name}|" "$sbatch_hydro"
    sed -i "s|^#SBATCH --output=.*|#SBATCH --output=${main_dir}/hydro_results/patches/out_files/${subfolder_name}.out|" "$sbatch_hydro"

    # Update the line where the python script is run
    sed -i "s|^python -m hydrophobicity.hydrophobia /vast/palmer/scratch/ohern/as4643/Decoys/Supersampled_structures/groups/9/ .*|python -m hydrophobicity.compare_chains ${subfolder}|" "$sbatch_hydro"

    # Verify changes
    echo "Updated sbatch file for $subfolder_name:"
    grep "^#SBATCH --job-name=" "$sbatch_hydro"
    grep "^#SBATCH --output=" "$sbatch_hydro"
    grep "^python /vast/palmer/scratch/ohern/as4643/Protein-Decoy-Detection-SVR/hydrophobicity/compare_chains.py" "$sbatch_hydro"
