...
def main(full_dir: str):
    ...
    for idx, all_contacts_df in enumerate(all_contacts_dfs):
        ...
        if interface_rsm_df is not None and interface_flatness_df is not None and hydrophobicity_df is not None:
            print(f"Merging set {idx+1}")
            try:
                merged_df = pd.merge(all_contacts_df, pd.merge(interface_rsm_df, pd.merge(interface_flatness_df, hydrophobicity_df, on='pdb_file'), on='pdb_file'), on='pdb_file')
