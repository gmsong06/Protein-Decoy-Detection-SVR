...
def main():
    """
    Reformats patch data (issue with how columns are made)
    """
    ...
    for data in os.listdir(args.data_folder):
        if data.endswith('.csv'):
            ...
            df[['patch_alignment_score', 'chain_patch_consistency_score', 'avg_patch_score']] = df[
                'patch_alignment_score'].str.extract(r'\(([^,]+), ([^,]+), ([^,]+)\)')
