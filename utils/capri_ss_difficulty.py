...
def compare_seeded_runs(targids, showplots):
    seeded_path_main = Path(
        'C:/Users/brand/Documents/OHern_Lab/DeepRank_Retrain/lr5e-6_epoch_comparisons/nonshuffled_scores/')
    ...
    ax = fig.add_axes([0.1, 0.11, 0.6, 0.75])
    for ind, score in enumerate(score_list):
        ax.plot(range(len(targids)), spearmans[score], color=colors[ind], marker=shapes[ind], markerfacecolor='None',
                linestyle='None', label=score, markersize=10)
    ...
    ax.hlines([-1, -0.8, -0.6, -0.4, -0.2, 0, 0.2, 0.4], 0, len(targids) - 1, colors='tab:gray', linestyles='dotted',
              linewidth=.5)
    ax.set_xticks(range(len(targids)))
...
def main():
    targids = ['T30', 'T32', 'T35', 'T37', 'T39', 'T41', 'T46', 'T47', 'T50', 'T53', 'T54']
    plot_all_scores(targids, 'data/capri_scoreset_difficulty_all.csv')
